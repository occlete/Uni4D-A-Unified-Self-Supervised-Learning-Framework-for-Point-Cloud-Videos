...
from modules.transformerv3 import *
...
class ContrastiveLearningModel(nn.Module):
    def __init__(self, radius=0.1, nsamples=32, spatial_stride=32,                            # P4DConv: spatial
        ...
        self.encoder_transformer = TransformerV2(en_emb_dim, en_depth, en_heads, en_head_dim, en_mlp_dim, dropout=dropout1)
        ...
        if self.pretraining:
            ...
            self.decoder_pos_mask_embed = nn.Conv1d(in_channels=en_emb_dim, out_channels=de_emb_dim, kernel_size=1, stride=1, padding=0, bias=True)
            self.decoder_pos_mask = nn.Conv1d(in_channels=de_emb_dim, out_channels=4, kernel_size=1, stride=1, padding=0, bias=True)
            ...
            # points_predictor
            #self.points_center_predict=nn.Conv1d(en_emb_dim, 3, 1)
    ...
    def decoder_random_masking(self, x):
        B, G, _ = x.shape

        if self.decoder_mask_ratio == 0:
            return torch.zeros(x.shape[:2]).bool()

        self.num_mask = int(self.decoder_mask_ratio * G)
    ...
    def forward_encoder(self, x):
        ...
        if self.pretraining:
            ...
            pos_emb_mask = xyzts[bool_masked_pos].reshape(batch_size, -1, 4)
            
            pos_emb_vis = self.encoder_pos_embed(pos_emb_vis.permute(0, 2, 1)).permute(0, 2, 1)
            pos_emb_mask = self.encoder_pos_embed(pos_emb_mask.permute(0, 2, 1)).permute(0, 2, 1)
            ...
            "cross-attention:input:pos_emd_mask + embvis  Q=pos_emd_mask, KV=pos_emd_mask + embvis"
            "output:b l*masked D,经过一个分类头，用CDLoss还是L2loss呢？然后detach一下 "
            pos_emb_mask_predict = self.encoder_transformer(fea_emb_vis,pos_emb_mask)
            pos_emb_mask_predict = self.encoder_norm(pos_emb_mask_predict)
            #Loss_center=F.mse_loss(pos_emb_mask,pos_emb_mask_predict,reduction='mean')

            return fea_emb_vis, bool_masked_pos, xyzts, xyzs_neighbors, shot_descriptors,pos_emb_mask_predict#,Loss_center
    ...
    def forward_decoder(self, emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,pos_mask_prdict):
        emb_vis = self.decoder_embed(emb_vis)
        pos_mask = self.decoder_pos_mask_embed(pos_mask_prdict.permute(0,2,1)).permute(0,2,1)
        pos_mask=pos_mask.detach()
        #pred_mask_center=self.decoder_pos_mask(pos_mask.permute(0,2,1)).permute(0,2,1)
        ...
        #Loss_center = self.criterion_dist(pred_mask_center,pos_emd_mask)
        #_,E_N_masked,_ = pos_emd_mask.shape
        #decoder_mask_pos=self.decoder_random_masking(pos_emd_mask)
        #decoder_pos=pos_emd_mask[decoder_mask_pos].reshape(batch_size, -1, 4)
        #print(decoder_mask_pos.shape)
        ...
        #Loss_center=F.mse_loss(pos_emd_mask,pos_mask,reduction='mean')
        #pos_emd_mask = self.decoder_pos_embed(decoder_pos.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        pos_all = torch.cat([pos_emd_vis, pos_mask+pos_emd_mask], dim=1)
        ...
        """
        loss = point_loss#+0.1*Loss_center#+ shot_loss
    ...
    def forward(self, clips):
        # [B, L, N, 3]
        if self.pretraining:
            emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,pos_emb_mask_predict = self.forward_encoder(clips)

            if self.vis:
                pre_points, gt_points, vis_points, mask = self.forward_decoder(emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,pos_emb_mask_predict)
                return pre_points, gt_points, vis_points, mask
            else:
                loss = self.forward_decoder(emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,pos_emb_mask_predict)
                return loss
        else:
