...
class ContrastiveLearningModel(nn.Module):
    ...
    def forward_encoder(self, x):
        ...
        if self.pretraining:
            ...
            return fea_emb_vis, bool_masked_pos, xyzts, xyzs_neighbors, shot_descriptors,x,xyzs_center
    ...
    def forward_decoder(self, emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,x,xyzs_center):
        ...
        if self.vis:
            vis_center = xyzs_center[~mask]#[B, L*N-m, 3]
            vis_points = xyzs_neighbors[~mask].reshape(batch_size, -1, self.tk, self.nsamples, 3) # [B, L*N-m, tk*nn, 3]
            #is_points = vis_points+vis_center.unsqueeze(2).unsqueeze(3)
            ...
            return pre_points, gt_points, vis_points, mask#,x
        else:
            return loss
    ...
    def forward(self, clips):
        # [B, L, N, 3]
        if self.pretraining:
            emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,x ,xyzs_center= self.forward_encoder(clips)

            if self.vis:
                pre_points, gt_points, vis_points, mask,= self.forward_decoder(emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,x,xyzs_center)
                return pre_points, gt_points, vis_points, mask
            else:
                loss = self.forward_decoder(emb_vis, mask, xyzts, xyzs_neighbors, shot_descriptors,x,xyzs_center)
                return loss
        else:
