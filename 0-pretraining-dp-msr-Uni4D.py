...
def train(model, optimizer, lr_scheduler, data_loader, 
    ...
    model.train()
    for i, (clips,text,index) in enumerate(data_loader):
        start_time = time.time()
        clips = clips.to(device)
        #clips_aug = clips_aug.to(device)
        #text = text.to(device)
        #text_feature = text_model(text)  # 在 text_model 内部分词并移动到设备
        #text_feature=text_feature.to(device)
        loss = model(clips,text)
        loss = loss.mean()
...
def main(args):
    ...
    model = ContrastiveMaskedAutoEncoder(
            ...
            momentum = args.momentum_encoder,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            torch.save(
            checkpoint,
            os.path.join(args.log_dir, 'checkpoint_{}.pth'.format(epoch))
            )
...
def parse_args():
    ...
    parser.add_argument('--temperature', default=0.1, type=float, help='Temperature parameter for contrastive learning')
    parser.add_argument('--momentum-encoder', default=0.999, type=float, help='Momentum parameter for the momentum encoder')
    parser.add_argument('--queue_size', default=12288, type=float, help='Momentum parameter for the momentum encoder')
    # input
    ...
    parser.add_argument('--log-dir', default='/home/pod/shared-nvme/Log_85mask', type=str, help='path where to save')
